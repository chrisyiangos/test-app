...
public class RfbEventResource {
    ...
    /**
     * SEARCH  /_search/rfb-events?query=:query : search for the rfbEvent corresponding
     * to the query.
     *
     * @param query the query of the rfbEvent search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/rfb-events")
    @Timed
    public ResponseEntity<List<RfbEventDTO>> searchRfbEvents(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of RfbEvents for query {}", query);
        Page<RfbEventDTO> page = rfbEventService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/rfb-events");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
