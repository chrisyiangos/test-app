...
public class RfbEventAttendanceResource {
    ...
    /**
     * SEARCH  /_search/rfb-event-attendances?query=:query : search for the rfbEventAttendance corresponding
     * to the query.
     *
     * @param query the query of the rfbEventAttendance search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/rfb-event-attendances")
    @Timed
    public ResponseEntity<List<RfbEventAttendanceDTO>> searchRfbEventAttendances(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of RfbEventAttendances for query {}", query);
        Page<RfbEventAttendanceDTO> page = rfbEventAttendanceService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/rfb-event-attendances");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
