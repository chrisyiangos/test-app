...
public class RfbLocationResource {
    ...
    /**
     * SEARCH  /_search/rfb-locations?query=:query : search for the rfbLocation corresponding
     * to the query.
     *
     * @param query the query of the rfbLocation search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/rfb-locations")
    @Timed
    public ResponseEntity<List<RfbLocationDTO>> searchRfbLocations(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of RfbLocations for query {}", query);
        Page<RfbLocationDTO> page = rfbLocationService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/rfb-locations");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
