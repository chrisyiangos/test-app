...
public class RfbUserResource {
    ...
    /**
     * SEARCH  /_search/rfb-users?query=:query : search for the rfbUser corresponding
     * to the query.
     *
     * @param query the query of the rfbUser search
     * @return the result of the search
     */
    @GetMapping("/_search/rfb-users")
    @Timed
    public List<RfbUserDTO> searchRfbUsers(@RequestParam String query) {
        log.debug("REST request to search RfbUsers for query {}", query);
        return rfbUserService.search(query);
    }
