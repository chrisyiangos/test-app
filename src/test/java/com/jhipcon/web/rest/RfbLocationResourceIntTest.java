...
public class RfbLocationResourceIntTest {
     ...
     *
     * @see com.jhipcon.repository.search.RfbLocationSearchRepositoryMockConfiguration
     */
    @Autowired
    private RfbLocationSearchRepository mockRfbLocationSearchRepository;
    ...
    public void createRfbLocation() throws Exception {
        ...
        // Validate the RfbLocation in Elasticsearch
        verify(mockRfbLocationSearchRepository, times(1)).save(testRfbLocation);
    }
    ...
    public void createRfbLocationWithExistingId() throws Exception {
        ...
        // Validate the RfbLocation in Elasticsearch
        verify(mockRfbLocationSearchRepository, times(0)).save(rfbLocation);
    }
    ...
    public void deleteRfbLocation() throws Exception {
        ...
        // Validate the RfbLocation in Elasticsearch
        verify(mockRfbLocationSearchRepository, times(1)).deleteById(rfbLocation.getId());
    }

    @Test
    @Transactional
    public void searchRfbLocation() throws Exception {
        // Initialize the database
        rfbLocationRepository.saveAndFlush(rfbLocation);
        when(mockRfbLocationSearchRepository.search(queryStringQuery("id:" + rfbLocation.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(rfbLocation), PageRequest.of(0, 1), 1));
        // Search the rfbLocation
        restRfbLocationMockMvc.perform(get("/api/_search/rfb-locations?query=id:" + rfbLocation.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(rfbLocation.getId().intValue())))
            .andExpect(jsonPath("$.[*].locationName").value(hasItem(DEFAULT_LOCATION_NAME.toString())))
