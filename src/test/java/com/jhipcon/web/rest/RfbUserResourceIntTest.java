...
public class RfbUserResourceIntTest {
     ...
     *
     * @see com.jhipcon.repository.search.RfbUserSearchRepositoryMockConfiguration
     */
    @Autowired
    private RfbUserSearchRepository mockRfbUserSearchRepository;
    ...
    public void createRfbUser() throws Exception {
        ...
        // Validate the RfbUser in Elasticsearch
        verify(mockRfbUserSearchRepository, times(1)).save(testRfbUser);
    }
    ...
    public void createRfbUserWithExistingId() throws Exception {
        ...
        // Validate the RfbUser in Elasticsearch
        verify(mockRfbUserSearchRepository, times(0)).save(rfbUser);
    }
    ...
    public void deleteRfbUser() throws Exception {
        ...
        // Validate the RfbUser in Elasticsearch
        verify(mockRfbUserSearchRepository, times(1)).deleteById(rfbUser.getId());
    }

    @Test
    @Transactional
    public void searchRfbUser() throws Exception {
        // Initialize the database
        rfbUserRepository.saveAndFlush(rfbUser);
        when(mockRfbUserSearchRepository.search(queryStringQuery("id:" + rfbUser.getId())))
            .thenReturn(Collections.singletonList(rfbUser));
        // Search the rfbUser
        restRfbUserMockMvc.perform(get("/api/_search/rfb-users?query=id:" + rfbUser.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(rfbUser.getId().intValue())))
            .andExpect(jsonPath("$.[*].username").value(hasItem(DEFAULT_USERNAME.toString())));
