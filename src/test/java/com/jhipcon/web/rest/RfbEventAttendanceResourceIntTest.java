...
@SpringBootTest(classes = MainappApp.class)
public class RfbEventAttendanceResourceIntTest {
    ...
    /**
     * This repository is mocked in the com.jhipcon.repository.search test package.
     *
     * @see com.jhipcon.repository.search.RfbEventAttendanceSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createRfbEventAttendance() throws Exception {
        ...
        // Validate the RfbEventAttendance in Elasticsearch
        verify(mockRfbEventAttendanceSearchRepository, times(1)).save(testRfbEventAttendance);
    }
    ...
    public void createRfbEventAttendanceWithExistingId() throws Exception {
        ...
        // Validate the RfbEventAttendance in Elasticsearch
        verify(mockRfbEventAttendanceSearchRepository, times(0)).save(rfbEventAttendance);
    }
    ...
    public void deleteRfbEventAttendance() throws Exception {
        ...
        // Validate the RfbEventAttendance in Elasticsearch
        verify(mockRfbEventAttendanceSearchRepository, times(1)).deleteById(rfbEventAttendance.getId());
    }

    @Test
    @Transactional
    public void searchRfbEventAttendance() throws Exception {
        // Initialize the database
        rfbEventAttendanceRepository.saveAndFlush(rfbEventAttendance);
        when(mockRfbEventAttendanceSearchRepository.search(queryStringQuery("id:" + rfbEventAttendance.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(rfbEventAttendance), PageRequest.of(0, 1), 1));
        // Search the rfbEventAttendance
        restRfbEventAttendanceMockMvc.perform(get("/api/_search/rfb-event-attendances?query=id:" + rfbEventAttendance.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(rfbEventAttendance.getId().intValue())))
            .andExpect(jsonPath("$.[*].attendanceDate").value(hasItem(DEFAULT_ATTENDANCE_DATE.toString())));
