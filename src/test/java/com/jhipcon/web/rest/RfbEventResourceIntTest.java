...
public class RfbEventResourceIntTest {
     ...
     *
     * @see com.jhipcon.repository.search.RfbEventSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createRfbEvent() throws Exception {
        ...
        // Validate the RfbEvent in Elasticsearch
        verify(mockRfbEventSearchRepository, times(1)).save(testRfbEvent);
    }
    ...
    public void createRfbEventWithExistingId() throws Exception {
        ...
        // Validate the RfbEvent in Elasticsearch
        verify(mockRfbEventSearchRepository, times(0)).save(rfbEvent);
    }
    ...
    public void deleteRfbEvent() throws Exception {
        ...
        // Validate the RfbEvent in Elasticsearch
        verify(mockRfbEventSearchRepository, times(1)).deleteById(rfbEvent.getId());
    }

    @Test
    @Transactional
    public void searchRfbEvent() throws Exception {
        // Initialize the database
        rfbEventRepository.saveAndFlush(rfbEvent);
        when(mockRfbEventSearchRepository.search(queryStringQuery("id:" + rfbEvent.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(rfbEvent), PageRequest.of(0, 1), 1));
        // Search the rfbEvent
        restRfbEventMockMvc.perform(get("/api/_search/rfb-events?query=id:" + rfbEvent.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(rfbEvent.getId().intValue())))
            .andExpect(jsonPath("$.[*].eventDate").value(hasItem(DEFAULT_EVENT_DATE.toString())))
